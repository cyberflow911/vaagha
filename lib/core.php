...
function prepareMessage3($msg,$title, $incentive,$id)
{
    ...
    $msg = str_replace('[LINK]',"<a href='localhost/vaagha/company/authentication?token=$id'>Click Here</a>",$msg);
    return $msg;  
}
...
//if admin already login
    function auto_redirect($conn)
    {
        if(isset($_SESSION['com_admin_signed_in']))
        {
            $email=$_SESSION['com_admin_signed_in'];
            $sql="select * from com_admins where email='$email'";
            $res=$conn->query($sql);
            if($res->num_rows > 0)
            {
                $row=$res->fetch_assoc(); 
...
function upload_tandc2($files,$conn,$table,$id_col,$column,$id,$images,$url)
{
	if(isset($_FILES[$images]))
    {
        $extension=array("docx", "txt", "pdf");
        foreach($_FILES[$images]["tmp_name"] as $key=>$tmp_name) 
        {
            $file_name=$_FILES[$images]["name"][$key];
            $file_tmp=$_FILES[$images]["tmp_name"][$key];
