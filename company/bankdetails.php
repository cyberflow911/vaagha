    ...
    if(isset($_GET['uid'])&& !empty($_GET['uid']))
    {
        ...
        if(isset($_POST['continue']))
        {
            if($user_details['pay_status']==1)
            {
                ...
                if($ctnc==1)
                {
                    $sql="insert into bank_details(u_id, p_id, user_name, bank_name,sort_code, account_num, com_tandc,p_tandc,p_tandc_date, status, tandc_date) values('$uid','$pid','$name', '$bank_name', '$sort_code', '$acc_num', 1,1,'$date',1, '$date')";
                    if($conn->query($sql))
                    {
                        $insert_id = $conn->insert_id;
                        $sql="update users set pay_status=2 where id='".$uid."'";
                        if($conn->query($sql))
                        {
                            $resMember=true;
                        }
                ...
                {
                    $sql="insert into bank_details(u_id, p_id, user_name, bank_name,sort_code, account_num, com_tandc,status, tandc_date) values('$uid','$pid','$name', '$bank_name', '$sort_code', '$acc_num', 1,1, '$date')";
                    if($conn->query($sql))
                    {
                        $insert_id = $conn->insert_id;
                        $sql="update users set pay_status=2 where id='".$uid."'";
            ...
            {
                $errorMember = "Your Bank Details are already saved.";
            }
        }
    }
...
<body style="background-color: #e1f3ff;">
        ...
        ?>
            <center>
                <div style="margin-top: 10px;" class="alert alert-success"><strong>Success! </strong> your request successfully updated. </div> </center>
        <?php
            }
        ...
        ?>
          <center>  
                <div style="margin-top: 10px; margin-left: 180px; margin-right: 180px;" class="alert alert-danger"><strong>Error! </strong><?=$errorMember?></div> </center>
        <?php
                
            }
        ?>
    <div class="row"  >
        <div class="col-12 col-lg-9 mx-auto" style="margin-top: 20px;">
