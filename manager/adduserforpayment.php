<?php
    ...
    $sql = "select u.*, p.title as p_title, c.f_name as pmf_name, c.l_name as pml_name , p.termandcondition as tnc from users u, projects p, com_admins c where u.pm_id='$MANAGER_ID' and u.pm_id=p.pm_id and c.id=u.pm_id and u.p_id=p.id and p.pm_id=c.id and u.pay_status=6";  
    if($result =  $conn->query($sql))
    {
        if($result->num_rows)
        {
