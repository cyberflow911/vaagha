<?php
    ...
    if(isset($_GET['token'])&&!empty($_GET['token']))
    {
        ...
        {  
            ...
            $email=$_POST['useremail'];
            $project=$_POST['userproject'];
            echo $sql="update users set salutation='$salutation', f_name='$f_name', l_name='$l_name', email='$email', p_id='$project', m_num='$m_num', incentive='$incentive'  where id='$token'";
            if($conn->query($sql))
            {
                $resMember  = "true";
            }
        ...
        $sql = "select u.*, p.title, p.id as pid, p.project_reference as p_ref from users u, projects p where u.id='$token' and u.pm_id='$MANAGER_ID' and u.p_id=p.id";
        if($result = $conn->query($sql))
        {
            if($result->num_rows)
            {
                            ...
                            <div class="row">
                                <div class="col-md-6"> 
                                    <div class="form-group">
                                        ...
                                        <input type="number" style="font-size: 16px;" value="<?=$user_details['m_num']?>"  id="userm_num" onchange="check(this)" name="userm_num" class="form-control" value="0" required>  
                                    </div> 
                                </div>
                                <div class="col-md-6"> 
                                    <div class="form-group">
