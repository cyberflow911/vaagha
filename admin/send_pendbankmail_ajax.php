...
if(isset($_POST['sendMail2']) && isset($_POST['bankdetails']))
{
    $emails = json_decode($_POST['emailss']);
    $pms = "'".implode($emails,"','")."'";
    echo $sql="select u.*, p.title, c.m_num as pmm_num, c.f_name as pmf_name, c.l_name as pml_name from users u, projects p, com_admins c where u.id in ($pms) and u.p_id=p.id and u.pm_id=c.id and p.pm_id=c.id";
    $res =  $conn->query($sql);
    if($res->num_rows)
    {
    ...
    $sql="select * from email where id='4'";
    $res =  $conn->query($sql);
    if($res->num_rows)
    {
        $row = $res->fetch_assoc();
    ...
    {
        echo $sub=prepareMessage($useremail['subject'],$data['title']);
